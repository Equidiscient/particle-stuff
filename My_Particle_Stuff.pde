...
public circleProperties[] circleLocations = new circleProperties[80]; //change this value to change the amount of circles
void setup(){
  ...
  for(int x = 0; x < circleLocations.length - 1; x++){
    for(int y = x + 1; y < circleLocations.length; y++){
      if(makePositive((circleLocations[x].X - circleLocations[y].X)) + makePositive((circleLocations[x].Y - circleLocations[y].Y)) < circleProx){
        stroke(
        (
        ((255 + backgroundShade) - ((float)((makePositive((int)(circleLocations[x].X - circleLocations[y].X)) + makePositive((int)(circleLocations[x].Y - circleLocations[y].Y)))) * lineStrokeC))
        )
        );
